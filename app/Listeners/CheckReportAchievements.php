...
class CheckReportAchievements
{
    /**
     * Handle the event.
     */
    public function handle(ReportViewed $event): void
    {
        // A lógica principal agora é só chamar a função auxiliar
        $this->awardAchievement($event->user, 'o-analista', fn() => true);
    }
    ...
    private function awardAchievement(User $user, string $slug, \Closure $condition): void
    {
        ...
        // 3. Condição específica para ganhar foi atendida?
        if ($condition()) {
            $user->achievements()->attach($achievement->id);
            Log::info("Usuário {$user->id} desbloqueou: {$achievement->name}");

            // ✅ Salva o OBJETO na sessão (vale só para a resposta atual)
            session()->now('new_achievement', $achievement); // <-- use 'now' aqui!
        }
    }
}
