...
<div class="max-w-4xl mx-auto bg-white p-8 rounded-2xl shadow-lg border border-gray-100 space-y-8">
    ...
    {{-- ATUALIZAÇÃO: O x-data agora controla o estado do tipo de lançamento, inicializando com o valor atual --}}
    <form action="{{ route('lancamentos.update', $lancamento) }}" method="POST" x-data="{ type: '{{ old('type', $lancamento->type) }}' }">
        @csrf
        @method('PATCH')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div>
                ...
                <div class="entry-type-toggle flex rounded-lg border border-gray-300 overflow-hidden">
                    {{-- ATUALIZAÇÃO: @click atualiza a variável 'type' do Alpine.js --}}
                    <input @click="type = 'receita'" type="radio" id="receita" name="type" value="receita" class="hidden"
                    ...
                    <input @click="type = 'despesa'" type="radio" id="despesa" name="type" value="despesa" class="hidden"
            ...
            {{-- Meta --}}
            <div class="md:col-span-2">
                <label for="meta_id" class="block text-sm font-semibold text-gray-700 mb-2">Vincular a Meta (Apenas para Receitas)</label>
