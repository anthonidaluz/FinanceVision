...
@section('content')
    {{-- A tag <style> que era específica para o gráfico donut deve ser movida para resources/css/app.css --}}
    {{-- Se ainda não fez, adicione o código no final de app.css e reinicie o 'npm run dev' --}}
    {{--
        @property --p {
          syntax: '<number>';
          inherits: false;
    ...
    <div class="bg-white p-6 rounded-xl shadow-md mb-8">
        <form action="{{ route('metas.index') }}" method="GET" class="flex flex-col sm:flex-row items-center gap-4">
            ...
            <div class="w-full sm:w-1/3">
                ...
                <select name="sort" id="sort" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm rounded-md">
            ...
            <div class="w-full sm:w-auto mt-4 sm:mt-0 sm:self-end">
                <button type="submit" class="w-full sm:w-auto inline-flex items-center justify-center px-6 py-2 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-primary hover:bg-opacity-90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
                    Filtrar
                </button>
            </div>
        </form>
    ...
    <main class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        @forelse ($metas as $meta)
            @php
                ...
                if ($isCompleted) $borderColorClass = 'border-green-500';
                if ($isOverdue) $borderColorClass = 'border-red-500';
            @endphp
            <article class="bg-white rounded-xl shadow-md border-t-4 {{ $borderColorClass }} p-6 flex flex-col sm:flex-row gap-6">
                <div class="flex-grow flex flex-col">
                    <div>
                        ...
                        <div class="text-sm space-y-2 mt-4 text-gray-600">
                            ...
                            <p><strong class="font-medium text-gray-700">Objetivo:</strong> R$ {{ number_format($meta->target_amount, 2, ',', '.') }}</p>
                            <p><strong class="font-medium text-gray-700">Poupado:</strong> R$ {{ number_format($meta->current_amount, 2, ',', '.') }}</p>
                            <p><strong class="font-medium text-gray-700">Prazo:</strong> {{ $meta->target_date ? \Carbon\Carbon::parse($meta->target_date)->format('d/m/Y') : 'Sem prazo' }}</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-2 mt-6 pt-4 border-t border-gray-200 mt-auto">
                        <a href="{{ route('metas.edit', $meta) }}" class="inline-flex items-center px-4 py-2 bg-primary border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-opacity-90">Editar</a>
                        <form action="{{ route('metas.destroy', $meta) }}" method="POST" onsubmit="return confirm('Tem certeza que deseja excluir esta meta?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-50">Excluir</button>
                        </form>
                    </div>
                </div>
                <div class="flex-shrink-0 flex flex-col items-center justify-center gap-2">
                    <div class="donut-chart w-32 h-32 rounded-full relative flex justify-center items-center" style="--p: {{ $meta->progress }}; background: conic-gradient(var(--success, #28a745) 0% calc(var(--p) * 1%), var(--danger, #e74c3c) calc(var(--p) * 1%) 100%);">
        ...
        <a href="{{ route('metas.create') }}" class="border-2 border-dashed border-gray-300 rounded-xl flex flex-col items-center justify-center text-gray-500 hover:bg-gray-50 hover:border-primary hover:text-primary transition-all min-h-[200px] py-10">
